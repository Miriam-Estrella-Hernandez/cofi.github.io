...
try {
    ...
    if (isset($_POST['area'], $_POST['fecha'], $_POST['hora'], $_POST['solicitante'], $_POST['motivo'])) {
        $area = $_POST['area'];
        $fecha = $_POST['fecha'];
        $hora = $_POST['hora'];
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO reservas (area, fecha, hora, solicitante, motivo) VALUES (:area, :fecha, :hora, :solicitante, :motivo)");
            $stmt->execute([
                'area' => $area,
                'fecha' => $fecha,
                'hora' => $hora,
...
<body>
    ...
    <nav>
        <ul>
            ...
            <li><a href="#" onclick="openTab(event, 'reservas')" class="tab-link">Reservas</a></li>
    ...
    <section id="blog" class="tab-content">
        ...
        <form action="profesor.php" method="POST">
            <input type="text" name="blog_title" required placeholder="Título del artículo">
            <textarea name="blog_content" required placeholder="Escribe el contenido del artículo..."></textarea>
    ...
    <section id="materiales" class="tab-content">
        ...
        <div id="materiales-content">
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                echo "<p><strong>" . htmlspecialchars($row['nombre']) . " (" . htmlspecialchars($row['tipo']) . ")</strong> <a href='" . htmlspecialchars($row['ruta']) . "' target='_blank'>Descargar</a></p>";
            }
            ?>
        </div>
        <form action="profesor.php" method="POST" enctype="multipart/form-data">
            <input type="text" name="material_name" required placeholder="Nombre del material">
   ...
   <section id="reservas" class="tab-content">
    ...
    <form action="profesor.php" method="post">
        ...
        <select name="area" id="area" required>
            <option value="Laboratorio De Computo">Laboratorio De Computo</option>
            <option value="Laboratorio De Redes">Laboratorio De Redes</option>
            <option value="Laboratorio Informático">Laboratorio Informático</option>
            <option value="Laboratorio Genérico">Laboratorio Genérico</option>
            <option value="Laboratorio Especializante">Laboratorio Especializante</option>
            <option value="Aula Amplia">Aula Amplia</option>
        ...
        <input type="text" name="solicitante" id="solicitante" required placeholder="Ingresa tu nombre">

        <label for="motivo">Motivo:</label>
        <textarea name="motivo" id="motivo" required placeholder="Describe el motivo de la reserva"></textarea>
    ...
    <!--<section id="reservas" class="tab-content">
    <h2>Reservas de Áreas</h2>
    <p>Aquí puedes gestionar las reservas de áreas para actividades educativas.</p>-->
